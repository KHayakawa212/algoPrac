...
class SLStack {
   ...
   */
  copy() {
    let stack_copy = new SLStack();
    let stack_copy2 = new SLStack();
    let runner = this.head;
    while (runner != null) {
      stack_copy.push(runner.value);
      runner = runner.next;
    }
    // stack_copy.reverse();
    // return stack_copy;
    runner = stack_copy.head;
    while (runner != null) {
      stack_copy2.push(runner.value);
      runner = runner.next;
    }
    return stack_copy2;
  }
   ...
   */
  partitionPositives() {
    let pos_queue = new SLQueue();
    let neg_queue = new SLQueue();
    let result_queue = new SLQueue();
    let runner = this.head;
    while (runner != null) {
      if (runner.value > 0) {
        pos_queue.enqueue(runner.value);
      } else {
        neg_queue.enqueue(runner.value);
      }
      runner = runner.next;
    }
    runner = pos_queue.head;
    while (runner != null) {
      result_queue.enqueue(runner.value);
      runner = runner.next;
    }
    runner = neg_queue.head;
    while (runner != null) {
      result_queue.enqueue(runner.value);
      runner = runner.next;
    }
    this.head = result_queue.head;
    return this;
  }
}
...
console.log(my_list);
my_list.printStack();
let part_list = new SLStack();
part_list = my_list.partitionPositives();
part_list.printStack();
