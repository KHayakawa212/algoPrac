...
function getMaxServings(recipe, available) {
  ...
  for (ingredient of recipeKeys) {
    if (available.hasOwnProperty(ingredient)) {
      findLimitingIngred.push(available[ingredient] / recipe[ingredient]);
    }
  }
  let min = findLimitingIngred[0];
  for (let i = 0; i < findLimitingIngred.length; i++) {
    if (findLimitingIngred[i] < min) {
      min = findLimitingIngred[i];
    }
  }
  console.log(findLimitingIngred);
  return `you can make ${min} servings of this recipe`;
}
