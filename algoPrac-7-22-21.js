...
//         rangeArray.push([Math.max(i+1-arr[i], 1), Math.min(i+1+a[i], arr.length)])
//     }
...
//-------------------------ATTEMPT 3 REFACTORING ATTEMPT 2------------------------------------------------------------------------
function gardens2(arr) {
  ...
  for (let i = 1; i < arr.length; i++) {
    if (
      Math.max(i - arr[i], 0) <= mintemp &&
      Math.min(i + arr[i], arr.length - 1) > maxtemp
    ) {
    ...
    // else if(Math.min(i+arr[i],arr.length - 1) > maxtemp && Math.max(i-arr[i], 0) > mintemp) {
